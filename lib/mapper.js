...
function Mapper (router, options) {
  ...
  HTTP_VERBS.forEach(function (VERB) {
    member[VERB] = function (routePath, filePath) {
      self.get(routePath, filePath, {member: true});
    };
    collection[VERB] = function (routePath, filePath) {
      self.get(routePath, filePath, {collection: true});
    };
  });
...
p.namespace = function (namespace, options, fn) {
  ...
  options || (options = {});
  var filePathNamespace = options.filePathNamespace;
...
p.setupRoute = function (route, options) {
  ...
  if (options.resourceName && !route.collectionName) {
    route.collectionName = options.resourceName;
  }

  if (route.collectionName && !route.resourceName) {
    route.resourceName = lingo.en.singularize(route.collectionName);
  }
